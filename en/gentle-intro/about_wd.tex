...
\item Cyc is a project of Cycorp (company) to create an ontological knowledge base 
    that allows solving problems in the field of artificial intelligence~\sidecite{Cyc}. 
...
\item Evi (formerly True Knowledgee~\sidecite{True_Knowledge}) 
    is a technology company in Cambridge, England that specializes in knowledge base 
        and 
        \index{Computer Science!Semantic search}
        \textit{semantic search}\sidenote[][]{%
%
Semantic search is a method and technology of information retrieval 
        based on the use of the context (semantic) meaning of the requested phrases, 
        instead of the dictionary meanings of individual words or expressions in a search query.%
%
} software. 
...
\item DBPedia is a crowdsourcing project aimed at extracting structured information 
    from data created by the Wikipedia project 
        ...
        for implementing linked data.\sidenote[][]{%
%
See about linked data and knowledge graph in the chapter ``\nameref{ch:ch-bb}'' p.~\pageref{ch:ch-bb}.%
%
} The project was started by recruiting volunteers 
...
linked to each other using properties.\sidenote[][]{%
%
For example, the following properties exist: \wdProperty{31}{instance of}, 
\wdProperty{279}{subclass of}, \wdProperty{361}{part of}, \wdProperty{527}{has part}.
%
} The power of Wikidata in its large volume, surprisingly rapid growth and self-organization; 
...
The article~\sidecite{Falcon_2.0} provides an example of using Wikidata 
...
\textit{Bots}\sidenote[][]{%
%
For more information on bots, see the chapter ``\nameref{ch:bots}'' on page \pageref{ch:bots}.} are a special tool, which is used in both Wikidata and Wikipedia. These are pieces of software that can automatically perform various actions on the platform at high speed (over a thousand edits per minute). Their main task is to edit existing data, add and import new data from other resources. Bots create reports which help the users to correct inaccuracies.
