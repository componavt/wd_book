...
\caption{Количество классов, сопутствующих <<населённому пункту>>, в~2017 и 2021 годы, их разница ($\Delta$). 
    Получены данные по всему миру. В таблице представлены первые строки ответа на запрос~\protect\ref{lst:human-settlement8}.}
...
\begin{lstlisting}[ language=SPARQL, 
                    caption={\href{https://w.wiki/4yyi}{Список классов поселений и их количество для объектов, имеющих свойство <<численность населения>> в России}\protect\footnotemark},
...
SELECT ?class ?classLabel (COUNT(?class) AS ?count) WHERE {
  SELECT ?class ?classLabel WHERE {
    [] wdt:P31 wd:Q486972; # noname instance of settlement
         wdt:P17 wd:Q159;  # in Russia
         wdt:P1082 [];     # has ?population
         wdt:P31 ?class.   # instance of ?class
...
ORDER BY DESC (?count)
\end{lstlisting}%
\footnotetext{Получили 36 разных классов поселений. Ссылка на SPARQL-запрос: \href{https://w.wiki/4yyi}{https://w.wiki/4yyi}}

Основные классы, полученные с помощью запроса~\ref{lst:human-settlement4}, 
представлены в таблице~\ref{tab:human-settlement1}. 

Строка шесть в~запросе~\ref{lst:human-settlement4} 
(\lstinline|wdt:P1082 ?pop;|) 
...
не заполнено поле <<численность населения>>\sidenote{%
%
Задача. Используя команду FILTER~NOT~EXISTS, 
измените запрос~\ref{lst:human-settlement4} так, чтобы получить число классов 
...
в строках 4 и 6 запроса~\ref{lst:human-settlement4}. 
...
Результаты построенных выше запросов 
показывают, что объект Викиданных \wdqName{населённый пункт}{486972} 
используется совместно с разными классами поселений. 
...
Итак, в городских поселениях живёт в четыре раза больше людей, чем в сельских. 
