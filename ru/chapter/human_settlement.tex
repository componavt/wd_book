...
\marginnote{Команда \lstinline|DISTINCT| в запросе~\ref{lst:human-settlement-count-total-city} 
нужна потому, что один населённый пункт 
может иметь несколько классов в поле \wdProperty{31}{экземпляр}, 
то есть его число жителей будет учитываться 
для нескольких разных строк в таблице~\ref{tab:human-settlement1}
Тогда суммарная численность будет неверной.}
...
Общее число составило $55,6 + 124,9 = 180,5$, 
при этом по по оценке Росстата в России было 145 млн человек на 2022 год. 
...
что большие города включают в себя множество небольших городов. 
Команда \lstinline|DISTINCT| в запросе~\ref{lst:human-settlement-count-total-city} 
эту проблему не решает, поскольку это разные объекты. 
Один из вариантов решения заключается в том, 
чтобы дополнительно выполнять проверку для населённых пунктов, 
не входят ли они в более крупные н.~п., и если входят, то не считать их население отдельно. 
