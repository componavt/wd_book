...
\end{lstlisting}%
\footnotetext{Получено 24797 учёных на~2022 год. Ссылка на SPARQL-запрос: \url{https://w.wiki/537r}}
...
Этот запрос интересн тем, что результаты одного запроса SELECT 
передаём на вход следующему запросу с помощью конструкции WITH { } AS ... INCLUDE.

\index{График!BarChart!Диаграмма отечественных учёных с разбивкой по годам рождений, родившихся в сельских поселениях}
\begin{lstlisting}[ language=SPARQL, 
...
ORDER BY ?year_lapse
\end{lstlisting}%
\footnotetext{Получено \num{99} записей на 2022 год. Ссылка на SPARQL-запрос: \href{https://w.wiki/53xM}{https://w.wiki/53xM}}
