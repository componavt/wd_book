...
построим диаграмму учёных, родившихся в селе, с разбивкой по десятилетиям %(?time_lapse) 
и научным направлениям. 
Этот запрос интересен тем, что результаты одного запроса \lstinline|SELECT| 
передаём на вход следующему запросу с помощью конструкции \lstinline|WITH { } AS ... INCLUDE|. 

Запрос~\ref{lst:scholar-degree} как раз и является 
первой частью запроса запроса~\ref{lst:scholar-village} (строки 2--12). 
...
участвуют в построении графика~\ref{fig:scholars-village-color}
...
ORDER BY ?year_lapse
\end{lstlisting}%
\footnotetext{Получено \num{99} записей на 2022 год. Ссылка на SPARQL-запрос: \href{https://w.wiki/55VK}{https://w.wiki/55VK}}
\end{fullwidth}
