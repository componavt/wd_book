...
\begin{task}
	\label{answer:ship_1}
	Решение:
	\begin{lstlisting}[ language=SPARQL, caption={{\href{https://w.wiki/wXG}{Изображения кораблей, снятых в фильмах}}\protect\footnotemark}, label=lst:wide_ship, ]
# Images of ships used in movies
#defaultView:ImageGrid
SELECT DISTINCT ?film ?filmLabel ?ship ?shipLabel ?image
WHERE
{
	?film wdt:P31 wd:Q571. # is film	
	...
	\end{lstlisting}
	\footnotetext{Найдено пять изображений кораблей, снятых в кинолетнах, 2021 год. Ссылка на SPARQL-запрос: \href{https://w.wiki/wXG}{https://w.wiki/wXG}}
...
\begin{task}
	...
	\begin{marginfigure}[0.0cm]
		...
		\caption{Почтовая марка с изображением эсминца \ruwiki{https://w.wiki/vgE}{Гремящий}, почта СССР, 1982.}%
...
\begin{task}
	...
	\newthought{Примеры возможных решений через Викиданные. Данные могут отличаться от ответа выше из-за неполноты информации об объектах Викиданных, см. листинги~\ref{lst:long_ship} и~\ref{lst:wide_ship}.}
\begin{lstlisting}[ language=SPARQL, caption={{\href{https://w.wiki/wXL}{Самый длинный корабль}}\protect\footnotemark}, label=lst:long_ship, ]
...
\end{lstlisting}
\footnotetext{Самым длинным кораблем оказался \Wikiref{NMS Mircea} длиной в \num{36} метров. Ссылка на SPARQL-запрос: \href{https://w.wiki/wXL}{https://w.wiki/wXL}}
...
}. Ссылка на SPARQL-запрос: \href{https://w.wiki/wKv}{https://w.wiki/wKv}}
