...
\begin{task}
	\label{answer:ship_1}
	Решение:
	\begin{lstlisting}[ language=SPARQL, caption={{\href{https://w.wiki/wKs}{Изображения кораблей, которые снимались в фильмах}}\protect\footnotemark}, label=lst:wide_ship, ]
...
WHERE
{
	...
	\end{lstlisting}
	\footnotetext{Найдено 5 изображений кораблей, снимавшихся в фильмах, 2021 год. Ссылка на SPARQL-запрос: \href{https://w.wiki/wKs}{https://w.wiki/wKs}}

	\begin{lstlisting}[ language=SPARQL, caption={{\href{https://w.wiki/wKu}{Изображения кораблей, о которых писали в книгах}}\protect\footnotemark}, label=lst:wide_ship, ]
# Images of ships used in movies
#defaultView:ImageGrid
SELECT DISTINCT ?ship ?shipLabel ?image
WHERE
{
	...
	\end{lstlisting}
	\footnotetext{Найдено 8 изображений кораблей, о которых было написано в книгах, 2021 год. Ссылка на SPARQL-запрос: \href{https://w.wiki/wKu}{https://w.wiki/wKu}}
...
Внутренний SELECT в скрипте из листинга~\ref{long_ship} находит максимальную длину корабля, а внешний SELECT находить корабль с такой длиной.

\begin{lstlisting}[ language=SPARQL, caption={{\href{https://w.wiki/wKv}{Изображения кораблей отсортированные по ширине корабля}}\protect\footnotemark}, label=lst:wide_ship, ]
