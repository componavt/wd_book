...
\todoVlad{
Если взять и упростить запрос~\ref{lst:music _in_each_subclass} 
так, чтобы он выводил получаемые музыкальные композиции, 
то получим неприятную вещь, а именно в результаты попали 
экземпляры объекта ``incipit (Q1161138)'', поскольку он является подклассом ``musical work/composition''. 
Нужно добавить исключение из результатов экземпляров этого класса. 

    После предыдущего запроса~\ref{lst:music _in_each_subclass} 
