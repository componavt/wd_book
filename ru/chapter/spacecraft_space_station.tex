...
\caption
[Союз-19, 2021.\index{Иллюстрации для вопросов!Космические корабли!Союз-19}]
{
...
\wdProperty{31}{частный случай понятия}. В ходе работы было обнаружено, что текущие показатели росийской космонавтики по количеству запусков космических кораблей за последние годы соответствют показателям в СССР пятьдеят лет назад. 
...
\index{SPARQL!VALUES!Получение значений из переменных}
...
\caption
[Союз-T, 2021.\index{Иллюстрации для вопросов!Космические корабли!Союз-T}]
{
...
\caption
[Лунный корабль, 2021.\index{Иллюстрации для вопросов!Космические корабли!Лунный корабль}]
{
...
Логический оператор \lstinline|UNION|\index{SPARQL!UNION!Логический оператор для объединения объектов} в строках 5--6 
позволяет объединить советские и российские запуски космолётов. 

Если бы в запросе вместо переменной \lstinline|?lapse|\index{SPARQL!lapse!Переенная соержащая дату запуска} оставалась~--- \lstinline|?year|\index{SPARQL!year!Переменная для подсчёта запусков} (строки 3 и 10), 
то мы получили бы на рис.~\ref{fig:launchesRussia5years} 
число запусков за каждый год. 
Благодаря функции округления \lstinline|FLOOR()|\index{SPARQL!FLOOR()!Функция округления} 
и группировке%
\sidenote[][]{
%
...
и в переменную \lstinline|?quantity|\index{SPARQL!quantity!Переменная для подсчёта количества запусков} записывается число этих объектов, 
подсчитанное с помощью функции \lstinline|COUNT()|\index{SPARQL!COUNT()!Функция подсчёта значений}.

Для представления результатов в виде столбчатой диаграммы используется стиль отображения BarChart\index{SPARQL!BarChart!Предсталение результатов в формате столбчатой диаграммы}.
...
\index{SPARQL!STR!Функция преобразования переменной к строковому типу}
\begin{lstlisting}[ language=SPARQL, 
...
\index{График!BarChart!Количетво запусков космических кораблей по годам и странам}
\begin{figure*}[h!]
  ...
  \caption[Запуски кораблей во всём мире]{Визуализация количества запусков космических кораблей по годам и странам, построено с помощью скрипта~\protect\ref{lst:launchesWorld} в 2021 году.}
