...
\marginnote{
	...
	\begin{tabular}{ll}
        Язык & Разработчик\\
		\hline
        \href{https://www.wikidata.org/wiki/Q154755}{Ада} & \href{https://ru.wikipedia.org/wiki/Ишбиа,_Жан}{Жан Ишбиа} и С.\,Такер Тафт\\
        \href{https://www.wikidata.org/wiki/Q334879}{Erlang} & \href{https://ru.wikipedia.org/wiki/Мур,_Чарльз_(программист)}{Чарльз Мур}\\
        \href{https://www.wikidata.org/wiki/Q275472}{Форт} & \href{https://ru.wikipedia.org/wiki/Армстронг,_Джо_(программист)}{Джо Армстронг}, 
                                                                Роберт Вирдинг и шведская компания Ericsson\\
	\end{tabular}

    \vspace{3pt}
    См. ответ на с.~\pageref{answer:prog_lang_1}.
}
%
...
\end{lstlisting}
\footnotetext{Получено: 732 языка программирования в~2017 году и 1422 языка в~2020 году. 
Ссылка на~SPARQL-запрос: \href{https://w.wiki/uGs}{https://w.wiki/uGs}.}
...
%%
\section{Пермиссивные лицензии}
\index{Информатика!Пермиссивная лицензия}%
%%
...
\end{lstlisting}
\footnotetext{%
    Получено: 37 языков программирования, находящихся под пермиссивными лицензиями, в~2017 году 
              и 83 языка в~2020 году. 
...
\label{question:prog_lang_2}
    ...
    \vspace{5pt}
    См. ответ %~\ref{answer:prog_lang_2} 
    на с.~\pageref{answer:prog_lang_2}.
}
...
\index{Информатика!Расширение имени файла}%
...
\begin{marginfigure}
\centering
	\includegraphics[width=0.7\linewidth]{./chapter/programming_language/File_extensions_quantity_of_source_code_2017.png}
    \caption[Число форматов файлов для языков программирования, 2017 год.]
    {Соотношение числа разных форматов файлов исходного кода\\по языкам программирования, 2017 год}
%            Размер пузырька соответствует числу форматов для одного языка. 
%            Ссылка на~SPARQL-запрос: \href{https://w.wiki/uGQ}{https://w.wiki/uGQ}}
...
\centering
	\includegraphics[width=0.74\linewidth]{./chapter/programming_language/File_extensions_quantity_of_source_code_2020.png}
	\caption[Число форматов файлов для языков программирования, 2020 год.]
    {Соотношение числа разных форматов файлов исходного кода\\для 108 языков программирования, 2020 год}
%    {Пузырьковая диаграмма с числом форматов файлов исходного кода для 108 языков программирования, 2020 год. Размер пузырька соответствует числу форматов для одного языка. Ссылка на~SPARQL-запрос: \href{https://w.wiki/uGQ}{https://w.wiki/uGQ}}
...
\centering
    ...
    {Место жительства Альберта Эйнштейна, 
     фрагмент страницы объекта \wdqName{Albert Einstein}{937}, 2023 год}
...
В~Викиданных это описывается свойством~\wdProperty{551}{residence}. 
%
...
\begin{compactitemize}
    ...
    \item язык программирования \wdqName{Factor}{1391724}
...
ORDER BY DESC(?count)
\end{lstlisting}
\footnotetext{Ссылка на~SPARQL-запрос: \href{https://w.wiki/94QJ}
                                            {https://w.wiki/94QJ}.}
...
\end{lstlisting}
\footnotetext{Результатом запроса будет карта, 
    на которой красными точками отмечены места расположения университетов, 
    в~которых учились люди, создавшие языки программирования. 
    На 2017 год получено 142 университета, к 2020 число записей увеличилось до 282.
    Ссылка на~SPARQL-запрос: \href{https://w.wiki/94hh}
                                  {https://w.wiki/94hh}.}
...
\end{lstlisting}
\footnotetext{Результатом запроса будет та~же карта, что и в предыдущем запросе, 
    но при наведении курсора на красную точку название университета не показывается, 
    поскольку переменной \texttt{?eduLabel} нет в~запрашиваемых результатах этого запроса.
    Ссылка на~SPARQL-запрос: \href{https://w.wiki/94hY}
                                  {https://w.wiki/94hY}.}
...
ORDER BY DESC(?count)
\end{lstlisting}
\footnotetext{Получено: 168 университетов в 2022 году и 396 университетов в~2024 году. 
    Ссылка на~SPARQL-запрос: \href{https://w.wiki/5CMt}{https://w.wiki/5CMt}.%
}
...
\marginnote[4\baselineskip]{
    \index{SPARQL![]!безымянная переменная}
    \MarginQuestion
    В~этом запросе переменная \lstinline|?developer| нужна только как~мостик, 
    чтобы соединить язык программирования (\mbox{\lstinline|?lang|}) 
    и~профессию разработчика (\mbox{\lstinline|?occupation|}). 
    По аналогии с~запросами~\ref{lst:developer_university} и~\ref{lst:developer_edu_coord} 
    преобразуйте этот запрос так, 
    чтобы сохранить упомянутый мостик, 
    но убрать переменную \mbox{\lstinline|?developer|}, 
    то~есть сделать её безымянной. 
    При этом запрос станет короче на~одну строчку.
}
%
\index{График!BubbleChart}
\begin{lstlisting}[
...
\footnotetext{Получено: 48 профессий в~2017 году и 74~профессии в 2020 году. 
              Ссылка на~SPARQL-запрос: \href{https://w.wiki/94jS}{https://w.wiki/94jS}.}


\begin{marginfigure}[0pt]
    \includegraphics[width=0.8\textwidth]{./chapter/programming_language/Bubble_chart_showing_the_quantity_of_professions_people,_creating_programming_languages_RU_2020.png}
    \vspace{-5pt}
...
\index{SPARQL!Label!SERVICE}
\index{SPARQL!Label!serviceParam}
\index{SPARQL!Label!rdfs:label}
...
\end{lstlisting}
\footnotetext{Получено: 150 разработчиков с~именами на~русском языке в~2023 году. 
              Ссылка на~SPARQL-запрос: \href{https://w.wiki/6gDx}{https://w.wiki/6gDx}.}
...
Ответ на~с.~\pageref{answer:prog_langs_4}.
	...
	\item Построить столбчатую диаграмму, отражающую количество известных хештегов\sidenote{%
%            
                \index{Информатика!Хештег}
                Хештег (\#)~--- ключевое слово, используемое в~микроблогах и~социальных сетях, 
