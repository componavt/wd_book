...
\index{График!BubbleChart!Форматы файлов кода}
\begin{lstlisting}[
	language=SPARQL,
...
\index{График!Map!Место жительства/работы разработчиков ЯП}
\begin{lstlisting}[
	language=SPARQL,
...
\index{График!BubbleChart!Благоприятные страны для разработчиков ЯП}
\begin{lstlisting}[
	language=SPARQL,
...
\index{График!Map!Университеты разработчиков ЯП}
\begin{lstlisting}[
	language=SPARQL,
	label=lst:developer_university,
	caption={\href{https://w.wiki/94Ac}
    {Карта с указанием университетов, в которых учились разработчики языков программирования}\protect\footnotemark},
	texcl,
    xleftmargin=18pt,
    numbers=left]
#defaultView:Map
SELECT ?langLabel ?developerLabel ?eduLabel ?coord WHERE {
  ?lang wdt:P31 wd:Q9143;    # instances of programming language
        wdt:P178 ?developer. # developed by developer
  ?developer wdt:P69 [wdt:P625 ?coord].   # who educated at ?edu
  SERVICE wikibase:label { bd:serviceParam wikibase:language "ru,en" }
}
\end{lstlisting}
\footnotetext{Результатом запроса будет карта, 
    ...
    Ссылка на~SPARQL-запрос: \href{https://w.wiki/94Ac}
                                  {https://w.wiki/94Ac}.}

\index{SPARQL![]!безымянная переменная}
Запрос~\ref{lst:developer_university} интересен использованием безымянной переменной 
в~строке~5 в~квадратных скобках. 
Развернём её и получим более простой запрос в два шага: 
(1) заменим квадратные скобки какой-либо переменной, обозначающей университет, например \lstinline|?edu|; 
(2) добавим после строки~5 утверждение, которое даёт нам координаты университета. 
Вот эти две строки вместо строки~5:\\
\lstinline|?developer wdt:P69 ?edu.|\\
\lstinline|?edu wdt:P625 ?coord.|
...
\index{График!BubbleChart!Университеты разработчиков ЯП}
\begin{lstlisting}[
	language=SPARQL,
...
\index{График!BubbleChart!Профессии создателей ЯП}
\begin{lstlisting}[
	language=SPARQL,
...
\marginnote[21pt]{%
        \index{Информатика!Хештег}Хештег (\#)~--- 
        ключевое слово, 
